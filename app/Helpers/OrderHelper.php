...
class OrderHelper
{
     ...
     * @return mixed
     */
    public static function actualQuantity(Stock $stock, mixed $quantity): mixed
    {
        ...
        if ($quantity < ($product?->min_qty ?? 0)) {

            $quantity = $product->min_qty;

        } else if($quantity > ($product?->max_qty ?? 0)) {
    ...
    private static function prepareByBonus(mixed $item, ?Stock $stock, int $quantity) {
        ...
        $item['total_price']  = $originPrice - $discount + $tax;
        $item['tax']          = $tax;
