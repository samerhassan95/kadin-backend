    ...
    <tbody>
        @foreach($products as $product)
            <tr>
                ...
                <td>{{ @($product['rate_tax'] * $product['quantity']) ?? 0 }}</td>
                <td>
                    {{ $position === 'before' ? $symbol : '' }}
                    {{ $product['rate_origin_price'] ?? 0 }}
                ...
                <td>
                    {{ $position === 'before' ? $symbol : '' }}
                    {{ @($product['rate_total_price'] * $product['quantity']) ?? 0 }}
                    {{ $position === 'after' ? $symbol : '' }}
                </td>
            </tr>
        @endforeach
...
<table class="table table-striped table-bordered">
    ...
    <tr>
        <th scope="row">
            {{ $position === 'before' ? $symbol : '' }}
            {{ ($order->rate_total_price - $order->rate_total_tax) }}
            {{ $position === 'after' ? $symbol : '' }}
        </th>
        <td>
            {{ $position === 'before' ? $symbol : '' }}
            {{ $order->rate_delivery_fee }}
            {{ $position === 'after' ? $symbol : '' }}
        </td>
        <td>
            {{ $position === 'before' ? $symbol : '' }}
            {{ $order->rate_total_price }}
            {{ $position === 'after' ? $symbol : '' }}
        </td>
        <td>
            {{ $position === 'before' ? $symbol : '' }}
            {{ $order->rate_coupon_price }}
            {{ $position === 'after' ? $symbol : '' }}
        </td>
        <td>
            {{ $position === 'before' ? $symbol : '' }}
